SKILL MAUI Debugging

*   https://github.com/shinyorg/mauiplatforms/blob/main/.claude/skills/maui-ai-debugging/SKILL.md
...
description: |
  ...
  1.  Building or running a MAUI app on iOS simulator, Android emulator, Mac Catalyst, macOS (AppKit), or Linux/GTK, 
  2.  Inspecting or interacting with a running app's UI (visual tree, tapping, filling text, screenshots, property
      queries), 
  3.  Debugging Blazor WebView content via CDP, 
  4.  Managing simulators or emulators, 
  5.  Setting up MauiDevFlow in a MAUI project, 
  6.  Completing a build-deploy-inspect-fix feedback loop, 
  7.  Handling permission dialogs and system alerts, (8) Managing multiple simultaneous apps via the broker daemon.
  ...
  * xcrun simctl,
  * xdotool, and 
  * dotnet build/run for all

MAUI target platforms including macOS (AppKit) and Linux/GTK.
---
...
```bash
dotnet tool uninstall   --global Redth.MauiDevFlow.CLI
dotnet tool install     --global Redth.MauiDevFlow.CLI
dotnet tool update      --global Redth.MauiDevFlow.CLI
# Android only
dotnet tool uninstall   --global androidsdk.tool    
# iOS/Mac only
dotnet tool uninstall   --global appledev.tools     
# Android only
